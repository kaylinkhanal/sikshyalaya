...
// import { , Container } from 'react-ify'
// import 'react-ify/dist/Reactify.css'
import { use } from "@/hooks/use-";
import { er } from "@/components/ui/er";
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-col gap-4">
        <Card className="bg-transparent border-black border border-opacity-30 w-1/3">
          <CardHeader>
            <CardTitle className="text-xl text-black bg-gray-200 p-2">
              Add New Class
            </CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={addClass} className="space-y-4">
              ...
              <Button
                type="submit"
                disabled={loading}
                className="rounded bg-black text-white mt-20"
                variant="outline"
              >
            ...
            {classes.length > 0 ? (
              <div className=" text-black flex gap-4 flex-wrap">
                {classes.map((item) => (
                  <Card
                    key={item._id}
                    onClick={() => router.push(`class/${item._id}/sections`)}
                    className="cursor-pointer w-[229px] hover:opacity-95 rounded-none"
                  >
                    <CardHeader>
