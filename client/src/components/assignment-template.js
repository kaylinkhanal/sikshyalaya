...
export function AssignmentTemplate({ questions, dueDate, courseName, teacherName,id,assignmentId }) {
  ...
  const handleScoreSubmission = async()=>{
   ...
   remarks: remarksRef.current.value
    ...
    await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/score-submission/${assignmentId}`,scoreDetails)
      ...
      {questions.map((question, index) => (
        <div key={index} className="mb-8 page-break-before">
          ...
          <div className="border border-gray-300 rounded-md">
            {params == '/teacher/assignment' ?    <div className='h-72 bg-white'>{question?.answer}</div> : <TiptapEditor answers={answers} setAnswers={setAnswers} question={question._id} id={id} isLastQuestion ={index === questions.length-1} /> }
          </div>
        </div>
      ))}
