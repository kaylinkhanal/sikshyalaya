...
export default function App() {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((studentList) =>
        studentList.name.toLowerCase().includes(filterValue.toLowerCase()),
      );
    }
    if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
      filteredUsers = filteredUsers.filter((studentList) =>
        Array.from(statusFilter).includes(studentList.status),
      );
    }
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-default-400 text-small">Total {studentList.length} Students</span>
