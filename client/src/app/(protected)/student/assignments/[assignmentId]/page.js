...
const Assignment = () => {
  ...
  const fetchAssignmentBySubjectId = async () => {
    const { data } = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/assignments?userId=${userDetails._id}&subjectId=${params.assignmentId}`
    );
    //console.log(data);
        ...
        >
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-4">
              {item.questionsSet?.length} Questions
            </p>
            ...
            <div className="flex items-center text-sm text-muted-foreground mt-2">
              ...
              <span>
                Due Date: {moment(item.dueDate).format("MMMM D, YYYY")}
              </span>
            </div>
          </CardContent>
        </Card>
